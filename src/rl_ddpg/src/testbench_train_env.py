...
class load_Motor:
    ...
    def calculate(self,force):
        self.force = force 
        self.voltage = self.force*135.0/40.0/14.3*self.p_desire/10.0/3
        return self.voltage

class ROS_env():
    def __init__(self):
        rospy.init_node('testbench_train_env', anonymous=True)
        ...
        # subscribe count 
        self.rackforce_reciever_callback_count = 0; 
    ...
    def rackforce_reciever_callback(self,data):
        self.rackforce_reciever_callback_count = self.rackforce_reciever_callback_count + 1
        if (self.rackforce_reciever_callback_count >= 100):
